...
def model_train(train_data_path = str, valu_data_path = str,  epoch = int, train_batch_size = int, model_save_path = str, train_max_length = int
  ...
  for epoch in range(epoch):
    ...
    running_loss = 0.0
    scd_train = Quora_Sent_Comp_Dataset(train_data_path)
    train_dataloader = DataLoader(scd_train, batch_size=train_batch_size, shuffle=True, collate_fn=Quora_collate_batch, drop_last=False)
    print('-- Starting Train')
    scd_valu = Quora_Sent_Comp_Dataset(valu_data_path)
    valu_dataloader = DataLoader(scd_valu, batch_size=train_batch_size, shuffle=True, collate_fn=Quora_collate_batch, drop_last=False)
  ...
  return train_loss
  #,valuation_loss, source_len,output_len,reference_len,ratio_OS, ratio_RS
